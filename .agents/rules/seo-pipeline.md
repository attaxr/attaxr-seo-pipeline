# SEO Content Pipeline — Agent Rules
...
```
Stage 1: Keyword Research → Stage 2: Content Collection → Stage 3: SEO Analysis → Stage 4: Content Creation
```

## Stage 1 — Keyword Research

Search domain keywords via web search. Capture term, search volume, difficulty, relevance (1-5), intent, cluster. Prioritize long-tail commercial. Save to `keywords.json`.

## Stage 2 — Content Collection

**Credential check (REQUIRED):**
1. Check `BROWSER_USE_API_KEY` → source `.env` → prompt user (redacted).
2. Never echo the key in any visible output.
...
Per article: keyword usage, gaps, missing subtopics, readability, structure, 3-5 improvements.
...
**Humanization:** strip AI-isms, adjust tone, vary rhythm, active voice.
