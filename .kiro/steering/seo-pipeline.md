# SEO Content Pipeline — Kiro Steering

> **Canonical source:** `AGENTS.md` (project root) — full pipeline details live there.

## Overview

Sequential 4-stage pipeline. Fail-stop on empty data. Runs on local files — no CMS.
...
Web search for domain keywords. Capture: term, search volume, difficulty, relevance (1-5), intent (commercial/informational/navigational), cluster. Prioritize long-tail commercial terms. Save to `keywords.json`.

## Stage 2 — Content Collection

**Credential check (REQUIRED):**
1. Check `BROWSER_USE_API_KEY` → source `.env` → prompt user with redacted input.
2. Never echo the key in any output.
3. Do not proceed without a valid key.

**Run:** `python -m seo_pipeline --from-json keywords.json`
**Fallback:** DuckDuckGo HTML (URL discovery only).
**Both fail:** Stop. Report whether `BROWSER_USE_API_KEY` was set.
